...
const findTodo = (todos, id) => todos.find((todo) => todo.id === id);
const findUser = (usernameOrId) => {
  const searchOption = validate(usernameOrId) ? "id" : "username";
  return users.find((user) => user[searchOption] === usernameOrId);
};
...
function checksCreateTodosUserAvailability(req, res, next) {
  const { user } = req;
  const todosCreated = user.todos.length;

  if (todosCreated === 10 && !user.pro) {
    return res.status(403).json({ error: "User cannot create more todos" });
  }
